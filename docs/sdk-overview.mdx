...
// Load from environment (mode is read from GLOSSO_MODE in .env)
...
```typescript
const tradingWallet = await GlossoWallet.fromEnv({ index: 1 });
const vaultWallet   = await GlossoWallet.fromEnv({ index: 2 });
```
